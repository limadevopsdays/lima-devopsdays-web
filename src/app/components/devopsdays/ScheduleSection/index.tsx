...
import { type ScheduleSession, type SessionType } from '../../../data/mockContent'
import { scheduleDay1I18n, scheduleDay2I18n } from '../../../data/mockContent.i18n'
import { useI18n } from '../../../i18n'
import { scheduleI18n } from './i18n'

// ─── Config per session type (static parts only) ─────────────────────────────
type TypeConfig = {
  label: string
  color: string
  bg: string
...
const typeBase: Record<SessionType, Omit<TypeConfig, 'label'>> = {
  keynote:   { color: '#f97316', bg: '#f9731618', Icon: Mic },
  talk:      { color: '#00d4ff', bg: '#00d4ff12', Icon: Mic },
  workshop:  { color: '#10b981', bg: '#10b98112', Icon: BookOpen },
  openspace: { color: '#6B51EF', bg: '#6B51EF12', Icon: Users },
  ignite:    { color: '#f59e0b', bg: '#f59e0b12', Icon: Zap },
  break:     { color: '#475569', bg: '#47556918', Icon: Coffee },
}
...
// ─── Session Row ──────────────────────────────────────────────────────────────
function SessionRow({ session, typeConfig }: { session: ScheduleSession; typeConfig: Record<SessionType, TypeConfig> }) {
...
export function ScheduleSection() {
  ...
  const day1Sessions = useI18n(scheduleDay1I18n)
  const day2Sessions = useI18n(scheduleDay2I18n)

  const typeConfig: Record<SessionType, TypeConfig> = Object.fromEntries(
    Object.entries(typeBase).map(([key, base]) => [
      key,
      { ...base, label: t.typeLabels[key as SessionType].label },
    ])
  ) as Record<SessionType, TypeConfig>

  const sessions = activeDay === 'day-1' ? day1Sessions : day2Sessions
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          {sessions.map((session, i) => (
            <SessionRow key={i} session={session} typeConfig={typeConfig} />
          ))}
        </div>
